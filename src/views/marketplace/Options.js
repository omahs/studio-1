...
const Options = ({ id, readOnly = false }) => {
	...
	useEffect(() => {
		if (value === "installed") {
			let usedList = [];
			if (search) {
				usedList = myTemplates.filter((temp) =>
					temp.schema.description.includes(search)
				);
			} else {
						...
						<Grid container spacing={1}>
							...
							{!isInstalled && !readOnly && (
								<Grid item xs={12}>
									...
									>
										Install Ver. {template?.schema?.version}
									</Button>
								</Grid>
							)}
							{isInstalled && !readOnly && (
